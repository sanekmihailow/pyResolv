...
- **resolve** — обогащает строки по ключевой колонке (`--key-column`, по
  ...
  **Прерывать безопасно:** Ctrl+C (или SIGTERM от cron) записывает в `-o` всё, что
  успели зарезолвить, печатает готовую команду продолжения и выходит с кодом 130;
  если подать этот файл обратно на вход, дорезолвится только недостающее (при
  сплите по подсетям в подсказке будут файлы подсетей). При прерывании `--delete`
  не срабатывает — входной файл остаётся.
...
- **Вариант B** (`run --config`) — быстрее (нет пересериализации между
  ...
  Подходит для данных, влезающих в RAM; для гигантских входов добавьте
  `--streaming`.
- **Вариант B + `--streaming`** — `run` с ограниченной памятью: сцепляет те же
  стадии через временные CSV-файлы в одном процессе, так что датасет никогда не
  лежит в RAM целиком (по сути «Вариант A внутри одного процесса»). Опционально;
  поведение `run` по умолчанию не меняется. Каталог для temp —
  `STREAMING__TEMP_LOG_PATH` (по умолчанию системный temp; размещайте на реальном
  диске, **не в tmpfs**), удаляется после прогона автоматически. Ставьте `resolve`
  **после** `aggregate` (он грузит кадр целиком); `--out-dir` — это финальный сплит
  **итогового** кадра, поэтому `aggregate --out-dir` с последующим `resolve` пишет
  **обогащённые** файлы по подсетям. Удобно для крона на очень больших входах:
...
`trim | aggregate --no-streaming` (и `run --streaming` совпадает с обоими).

Требует `PyYAML` (входит в `requirements.txt`).

## Продолжение прерванного резолва

`resolve` — самая долгая стадия, и прервать её можно без потери работы. Ctrl+C
(или SIGTERM, который шлют cron и systemd) отменяет очередь запросов, записывает
всё уже зарезолвленное ровно туда же, куда записал бы завершившийся прогон,
печатает команду продолжения и выходит с кодом **130**:

```
Резолвинг через rdap:  55%|#####5    | 33/60 [00:10<00:08]
^C
Прервано: зарезолвено 33 из 60 ключей. Частичный результат сохранён; при следующем запуске заполненные строки пропускаются.
Продолжить:
  pyresolv --type resolve -i out.csv -o out.csv --resolver rdap --key-column DstIP
```

Запускаешь эту команду — и в сеть уходят только недостающие ключи: заполненные
строки пропускаются (идемпотентность выше), остальное берётся из кэша, который
пишется по каждому ключу сразу. Прервать продолжение так же безопасно —
повторять можно сколько угодно раз.

**При разбивке по подсетям** (`aggregate: {out_dir: …}`) частичный результат — это
и есть сама разбивка, поэтому напечатанная команда обходит её файлы:
...
Уже полные файлы отработают мгновенно («Нечего обогащать»).

**После пропажи питания** обработчик не срабатывает, частичный CSV не пишется —
но переживают кэш (коммит на каждый ключ) и файлы завершённых стадий. После
`run --streaming` промежуточные CSV остаются на диске, потому что уборка
временного каталога не выполнялась; имена — `1_collect.csv`, `2_trim.csv`,
`3_aggregate.csv`, …:
...
`resume.yaml` — обычный пайплайн без уже отработавших стадий; какой именно, зависит
от самого свежего уцелевшего файла:

```yaml
# pipeline.yaml — обычный прогон
...
```yaml
# resume.yaml — продолжение с 2_trim.csv (collect и trim уже отработали)
- aggregate: {start: 5, end: 0, time_unit: d, min_count: 20}
- resolve: {resolver: default}
```

Если уцелел только `1_collect.csv`, добавь в начало `- trim`. `out_dir` в YAML
держать не обязательно — передай `--out-dir` флагом, чтобы разбивка легла в тот же
каталог отчёта, что и у прерванного прогона; а вот `start`/`end`/`time_unit` должны
совпадать с исходными, они формируют временной срез в именах файлов разбивки.

Задай `STREAMING__TEMP_LOG_PATH` на реальном диске: если `/tmp` смонтирован как
tmpfs, после пропажи питания не уцелеет ничего и `collect` придётся гонять заново.

Две вещи, о которых стоит помнить:

- **`--delete` несовместим с перезапуском** — он удаляет ровно те промежуточные
  файлы, за которые зацепился бы повторный прогон.
- **Не подавай агрегированный CSV в пайплайн, начинающийся с `aggregate`**:
  повторная агрегация сгруппированного файла обнулит `count` до 1 и потеряет
  колонки резолва, после чего фильтр `min_count` выкосит весь результат.
  Продолжай с trim-файла.
